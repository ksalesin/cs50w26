...
As you may know, it is common to digitally represent a color using RGB format for its Red, Green, and Blue components. Each channel is typically represented with a single integer from 0 to 255. The RGB value can also be combined into a single hex code: for example, a pure red color with RGB values (255, 0, 0) could also be represented with the hex code 0xFF0000, a pure green (0, 255, 0) would be 0x00FF00, and a pure blue (0, 0, 255) would be 0x0000FF.
